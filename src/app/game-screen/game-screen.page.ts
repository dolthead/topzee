...
export class GameScreenPage {
        ...
        this.gameService.game.dice.forEach(die => {
          if (!die.locked) {
            ...
            setTimeout(() => {
              ...
              if (
                this.scoringService.getOAKScore(this.gameService.game.dice, 5)
              ) {
  ...
  dieClick(i: number) {
    if (
      ...
      this.gameService.game.dice[i].locked = !this.gameService.game.dice[i]
        .locked;
      this.setRollLabel();
    }
  }
   ...
   */
  async setRollLabel() {
    this.unlockedCount = await this.gameService.game.dice.filter(d => !d.locked)
      .length;
